...
Purpose: Train a simple baseline model (Logistic Regression) on the Risk dataset (Diagnosis target), so later we can COMPARE it against the Deep Learning model. This proves whether deep learning actually performs better.
"""
import numpy as np
...
# Load the already-preprocessed data (created by preprocess_risk.py)
data = np.load('data/processed/risk_module_data.npz')
X_train, X_test = data['X_train'], data['X_test']
ydiag_train, ydiag_test = data['ydiag_train'], data['ydiag_test']

# Train Logistic Regression on the Diagnosis target (Benign/Malignant)
...
# Save the model so we can compare it later against the deep learning model
